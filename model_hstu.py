...
# ----- Old Model Architecture: LLaMA2 for Language Task -----
...
class LLaMA2_SASRec(nn.Module):
    ...
    # ----- Old Forward Function: LLaMA2 for Language Task -----
    ...
    # ----- Old Prediction Function: for Language Task (Actually Named Generate Function for Its Auto-regressive Property) -----
...
class HSTUAttention(nn.Module):
    ...
    def forward(
        ...
        xq, xk, xv, xu = self.wq(x), self.wk(x), self.wv(x), self.xu(x)
        xq = xq.view(bsz, seqlen, self.n_local_heads, self.head_dim)
        xk = xk.view(bsz, seqlen, self.n_local_kv_heads, self.head_dim)
        xv = xv.view(bsz, seqlen, self.n_local_kv_heads, self.head_dim)
        xu = xv.view(bsz, seqlen, self.n_local_heads, self.head_dim)
        ...
        # scores = F.softmax(scores.float(), dim=-1).type_as(xq)
        scores = scores.masked_fill(self.mask != 0, float("0")).type_as(xq)
